...
// now we make some change to our data:
...
// Slowly Changing Dimensions Type 2 (SCD2) logic to track inserts, updates, and deletes from a customer table into a customer_history table.
...
from ( -- we get all of this data from the change table we have generated.
select CUSTOMER_ID, FIRST_NAME, LAST_NAME, EMAIL, STREET, CITY, STATE, COUNTRY,
             update_timestamp as start_time,
             -- LAG() allows you to look at a previous row's value in a result set, without needing to self-join the table.
